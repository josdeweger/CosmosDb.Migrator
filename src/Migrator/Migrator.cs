...
public class Migrator : IMigrator
{
    ...
    public Migrator(Database db, ILogger logger, List<Type> migrationTypes, JsonSerializer? customSerializer = null)
    {
        _db = db;
        _logger = logger;
    ...
    /// <summary>
    /// Loops through all migrations added to this Migrator and:
    ...
    public async Task MigrateUp()
    {
        ...
        foreach (var migrationInfosForCurCollection in migrationsPerCollection)
        {
            ...
            foreach (var migrationInfo in migrationInfosForCurCollection)
            {
                ...
                var currentVersion = await GetCurrentVersion(migrationInfo.Migration.MigrationConfig);
    ...
    private async Task<long> GetCurrentVersion(MigrationConfig config)
    {
        ContainerProperties props = new(config.CollectionName, config.PartitionKeyPath);
        ...
        try
        {
            ...
            if (versionDocResponse.Resource?.Version is not null)
            {
                _logger.LogInformation("Current version {Version} on collection {CollectionName}", 
                    versionDocResponse.Resource.Version.Value, config.CollectionName);
                
                return versionDocResponse.Resource.Version.Value;
            }

            _logger.LogInformation("Current version {Version} on collection {CollectionName}", 0,
